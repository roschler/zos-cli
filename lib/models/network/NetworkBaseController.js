...
class NetworkBaseController {
  ...
  get networkPackage() {
    if (!this._networkPackage) {
      this._networkPackage = _zosLib.FileSystem.parseJsonIfExists(this.networkFileName) || _lodash2.default.cloneDeep(this.defaultNetworkPackage);
    }
  ...
  writeNetworkPackage() {
    _zosLib.FileSystem.writeJson(this.networkFileName, this.networkPackage);
  ...
  async uploadContracts(reupload) {
    const failures = [];
    await Promise.all((0, _lodash2.default)(this.packageData.contracts).toPairs().filter(([contractAlias, contractName]) => reupload || this.hasContractChanged(contractAlias)).map(([contractAlias, contractName]) => this.uploadContract(contractAlias, contractName).catch(error => failures.push({ contractAlias, error }))));
  ...
  async uploadContract(contractAlias, contractName) {
    ...
    this.networkPackage.contracts[contractAlias] = {
      ...
      bytecodeHash: (0, _digest.bytecodeDigest)(contractClass.bytecode)
    };
  }

  checkLocalContractsDeployed(throwIfFail = false) {
    const contracts = _lodash2.default.keys(this.packageData.contracts);
    let msg;

    const [contractsDeployed, contractsMissing] = _lodash2.default.partition(contracts, alias => this.isContractDeployed(alias));
    ...
    if (msg && throwIfFail) throw Error(msg);else if (msg) log.info(msg);
  }
  ...
  hasContractChanged(contractAlias) {
    ...
    const contractClass = _zosLib.Contracts.getFromLocal(contractName);
    const currentBytecode = (0, _digest.bytecodeDigest)(contractClass.bytecode);
  ...
  isContractDeployed(contractAlias) {
    return !this.isApplicationContract(contractAlias) || !_lodash2.default.isEmpty(this.networkPackage.contracts[contractAlias]);
  }
